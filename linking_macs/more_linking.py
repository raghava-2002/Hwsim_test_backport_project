...
def link_mac_addresses(station_mac_transitions, max_time_diff=1.0):
    ...
    linked_macs = []
    used_sequences = {}  # Dictionary to track linked sequences
    ...
    for i in range(len(mac_list)):
        ...
        # Initialize a set for each MAC in used_sequences if not already there
        if old_mac not in used_sequences:
            used_sequences[old_mac] = set()
        
        # Skip if the last sequence number has already been linked
        ...
        for j in range(len(mac_list)):
            if i != j:
                ...
                # Initialize used_sequences for new_mac if not present
                if new_mac not in used_sequences:
                    used_sequences[new_mac] = set()
                
                # Skip if the first sequence number has already been linked
                if new_first_seq in used_sequences[new_mac]:
                    continue

                time_diff = abs(new_first_time - old_last_time)

                # Only link if time difference is less than the threshold
                if time_diff < max_time_diff and time_diff < smallest_time_diff:
                    smallest_time_diff = time_diff
                    best_match = new_mac

        if best_match:
            # Link MACs and mark their sequences as used
...
if __name__ == "__main__":
    ...
    pcap_file = '/media/sf_rathan-dataset/pcap_files/linking/ap_trigger/test_3.pcap'  # Update with your pcap file path
    main(pcap_file)
