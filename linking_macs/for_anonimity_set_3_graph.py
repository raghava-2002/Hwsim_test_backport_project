...
matches = re.findall(r"Linked Old MAC: ([0-9a-f:]+) -> New MAC: ([0-9a-f:]+)", linking_log)
green_edges = []
red_edges = []
for old_mac, new_mac in matches:
    G.add_edge(old_mac, new_mac)
    same_set = any(old_mac in [base_mac] + macs and new_mac in [base_mac] + macs for base_mac, macs in mac_sets.items())
    (green_edges if same_set else red_edges).append((old_mac, new_mac))
...
# Manual time drift values in ms for each base MAC and its random MACs
manual_time_drifts = {
    "02:00:00:00:00:00": [1.9],
    "02:00:00:00:01:00": [-2.7],
    "02:00:00:00:02:00": [-5.2]
}
...
y_spacing = 0.7  # Adjust this to reduce white space
for base_mac, random_macs in mac_sets.items():
    pos[base_mac] = (0, y)
...
# Add station names and manual time drift annotations
for i, (base_mac, random_macs) in enumerate(mac_sets.items(), 1):
    plt.text(pos[base_mac][0] - 0.5, pos[base_mac][1], f'STA{i}', 
             horizontalalignment='right', verticalalignment='center', 
             fontsize=13, color='black')
    
    # Annotate manual time drift values
    drifts = manual_time_drifts.get(base_mac, [])
    for j, random_mac in enumerate(random_macs):
        if j < len(drifts):  # Check if drift value exists
            drift_value = drifts[j]
            drift_text = f"Time drift w.r.t AP: {'+' if drift_value >= 0 else ''}{drift_value:.1f}ms"
            plt.text(pos[random_mac][0], pos[random_mac][1] - 0.2, 
                     drift_text, horizontalalignment='center', 
                     verticalalignment='baseline', fontsize=13, color='black')
...
for i, random_mac in enumerate([last_base_mac] + last_random_macs, 1):
    plt.text(pos[random_mac][0], pos[random_mac][1] - 0.4, f'Cycle {i}', 
             horizontalalignment='center', verticalalignment='top', fontsize=13, color='black')

# Print the number of green and red edges on the graph
plt.text(0.95, 0.95, f"Correct links: {len(green_edges)}", 
         horizontalalignment='left', verticalalignment='top', 
         transform=plt.gca().transAxes, fontsize=12, color='green')
plt.text(0.95, 0.90, f"Incorrect links: {len(red_edges)}", 
         horizontalalignment='left', verticalalignment='top', 
         transform=plt.gca().transAxes, fontsize=12, color='red')
plt.text(0.95, 0.85, f"Accuracy: {((len(green_edges) / (len(green_edges) + len(red_edges))) * 100):.3f}%", 
         horizontalalignment='left', verticalalignment='top', 
         transform=plt.gca().transAxes, fontsize=12, color='green')
...
plt.ylabel("Stations")
plt.axis("off")
plt.savefig("mac_transitions_graph_with_time_drift.pdf", bbox_inches="tight", format="pdf")
