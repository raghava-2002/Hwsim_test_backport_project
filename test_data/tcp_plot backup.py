...
for scheme_name, directory in scheme_directories.items():
    ...
    plt.xticks(ticks=range(0, int(avg_data['seconds'].max()) + 1, 4))  # Set x-ticks every 5 seconds
    plt.legend()
    plt.grid(True)
    plt.savefig(f'{scheme_name}_tcp_bandwidth.png')
    plt.clf()  # Clear the plot for the next scheme

    # Plot Retransmissions over Time for the scheme
    plt.figure(figsize=(10, 6))  # Larger figure size for better x-axis spacing
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f'{scheme_name}_tcp_retransmissions.png')
    plt.clf()  # Clear the plot for the next scheme
