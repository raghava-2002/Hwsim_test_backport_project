...
# Function to plot a single metric with error bars
def plot_graph_with_error_bars(x_values, y_values_dict, ylabel, title, filename):
    ...
    for scheme, values in y_values_dict.items():
        ...
        # Plot the averaged line with error bars
        plt.errorbar(x_values, mean_values, yerr=std_dev, fmt='o', color=values['color'], label=scheme, capsize=5)
...
if plot_no_rnd:
    no_rnd_dir = os.path.join(base_dir, 'no_rnd', 'mode_n', 'without_wmd')
...
if plot_kernel_time:
    kernel_time_dir = os.path.join(base_dir, 'kernel_time', 'mode_n', 'without_wmd')
...
if plot_ap_trigger:
    ap_trigger_dir = os.path.join(base_dir, 'ap_trigger', 'mode_n', 'without_wmd')
...
# Plot TCP Throughput with Error Bars
plot_graph_with_error_bars(distances, {scheme: data['tcp_throughput'] for scheme, data in data_to_plot.items()},
                           'Throughput (Mbps)', 'TCP Throughput vs Distance from AP', f"{base_dir}/tcp_throughput_comparison_with_error_bars.pdf")
