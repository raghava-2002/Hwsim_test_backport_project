...
plot_all_tests = True  # Set to True to enable
...
def load_and_average_iperf3_results(scheme_dir):
    ...
    return (avg_tcp_retransmissions, avg_throughput_values_tcp, avg_udp_packet_loss_values, avg_udp_jitter_values, avg_throughput_values_udp, 
            ...
            tcp_retransmissions, throughput_values_tcp, udp_packet_loss_values, udp_jitter_values, throughput_values_udp)

# Function to plot with "Pointwise intervals" (error bars) and "Simultaneous intervals" (shaded confidence)
def plot_graph_with_intervals(x_values, y_values_dict, ylabel, title, filename, plot_all_tests):
    ...
    for scheme, values in y_values_dict.items():
        ...
        if plot_all_tests and 'individual_results' in values:
            for result in values['individual_results']:
                plt.plot(x_values, result, color=color, alpha=0.1)

        # Plot mean line with "Pointwise intervals" (error bars)
        plt.errorbar(x_values, mean_values, yerr=std_dev, fmt='o-', color=color, label=scheme, capsize=3, alpha=0.8)

        # "Simultaneous intervals" (shaded area for confidence interval)
        plt.fill_between(x_values, mean_values - std_dev, mean_values + std_dev, color=color, alpha=0.2)
...
if plot_no_rnd:
    ...
    std_tcp_retrans_no_rnd, std_throughput_tcp_no_rnd, std_udp_loss_no_rnd, std_udp_jitter_no_rnd, std_udp_throughput_no_rnd, \
    ...
    data_to_plot['Baseline (No MAC Randomization)'] = {
        'y': avg_tcp_throughput_no_rnd, 
        'std': std_throughput_tcp_no_rnd, 
        'color': scheme_colors['no_rnd'],
        'individual_results': all_tcp_throughput_no_rnd if plot_all_tests else None
    }
...
if plot_kernel_time:
    ...
    std_tcp_retrans_kernel_time, std_throughput_tcp_kernel_time, std_udp_loss_kernel_time, std_udp_jitter_kernel_time, std_udp_throughput_kernel_time, \
    ...
    data_to_plot['Kernel Time-Based MAC Randomization'] = {
        'y': avg_tcp_throughput_kernel_time, 
        'std': std_throughput_tcp_kernel_time, 
        'color': scheme_colors['kernel_time'],
        'individual_results': all_tcp_throughput_kernel_time if plot_all_tests else None
    }
...
if plot_ap_trigger:
    ...
    std_tcp_retrans_ap_trigger, std_throughput_tcp_ap_trigger, std_udp_loss_ap_trigger, std_udp_jitter_ap_trigger, std_udp_throughput_ap_trigger, \
    ...
    data_to_plot['AP-Initiated MAC Randomization'] = {
        'y': avg_tcp_throughput_ap_trigger, 
        'std': std_throughput_tcp_ap_trigger, 
        'color': scheme_colors['ap_trigger'],
        'individual_results': all_tcp_throughput_ap_trigger if plot_all_tests else None
    }

# Plot TCP Throughput with both "Pointwise intervals" and "Simultaneous intervals"
plot_graph_with_intervals(distances, data_to_plot,
                          'Throughput (Mbps)', 'TCP Throughput vs Distance from AP', 
                          f"{base_dir}/tcp_throughput_intervals.pdf", plot_all_tests)
