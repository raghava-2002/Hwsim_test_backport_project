...
# Toggle to control TCP and UDP plots
plot_tcp = False
plot_udp = True
...
def load_iperf3_results(test_path):
    tcp_retransmissions, throughput_values_tcp = [], []
    udp_packet_loss_values, udp_jitter_values, throughput_values_udp = [], [], []
...
def load_and_average_iperf3_results(scheme_dir):
    ...
    for test_dir in test_dirs:
        ...
        if plot_tcp and tcp_throughput:
            tcp_retransmissions.append(tcp_retrans)
            throughput_values_tcp.append(tcp_throughput)
            #need conversion do it here
            #throughput_values_tcp = [x * 125 for x in throughput_values_tcp] Mbps to KBps
        if plot_udp and udp_throughput:
            udp_packet_loss_values.append(udp_loss)
            udp_jitter_values.append(udp_jitter)
            throughput_values_udp.append(udp_throughput)

    # Calculate averages
    avg_tcp_retransmissions = np.mean(tcp_retransmissions, axis=0) if plot_tcp else []
    avg_throughput_values_tcp = np.mean(throughput_values_tcp, axis=0) if plot_tcp else []
    avg_udp_packet_loss_values = np.mean(udp_packet_loss_values, axis=0) if plot_udp else []
    avg_udp_jitter_values = np.mean(udp_jitter_values, axis=0) if plot_udp else []
    avg_throughput_values_udp = np.mean(throughput_values_udp, axis=0) if plot_udp else []
...
if plot_no_rnd:
    no_rnd_dir = os.path.join(base_dir, 'no_rnd', 'only_udp')
...
if plot_kernel_time:
    kernel_time_dir = os.path.join(base_dir, 'kernel_time', 'only_udp')
...
if plot_ap_trigger:
    ap_trigger_dir = os.path.join(base_dir, 'ap_trigger', 'only_udp')
...
# Plot TCP graphs if enabled
if plot_tcp:
    plot_graph(distances, {scheme: {'y': data['tcp_throughput'], 'color': data['color']} for scheme, data in data_to_plot.items()},
               'Throughput (Mbps)', 'TCP Throughput vs Distance from AP', f"{base_dir}/tcp_throughput_comparison_o.png",
    ...
    plot_graph(distances, {scheme: {'y': data['tcp_retrans'], 'color': data['color']} for scheme, data in data_to_plot.items()},
               'Retransmissions', 'TCP Retransmissions vs Distance from AP', f"{base_dir}/tcp_retransmissions_comparison_o.png",
               {scheme: individual_results[scheme]['tcp_retrans'] for scheme in individual_results})

# Plot UDP graphs if enabled
if plot_udp:
    plot_graph(distances, {scheme: {'y': data['udp_throughput'], 'color': data['color']} for scheme, data in data_to_plot.items()},
               'Throughput (Mbps)', 'UDP Throughput vs Distance from AP', f"{base_dir}/udp_throughput_comparison_o.png",
    ...
    plot_graph(distances, {scheme: {'y': data['udp_loss'], 'color': data['color']} for scheme, data in data_to_plot.items()},
               'Packet Loss (%)', 'UDP Packet Loss vs Distance from AP', f"{base_dir}/udp_packet_loss_comparison_o.png",
    ...
    plot_graph(distances, {scheme: {'y': data['udp_jitter'], 'color': data['color']} for scheme, data in data_to_plot.items()},
               'Jitter (ms)', 'UDP Jitter vs Distance from AP', f"{base_dir}/udp_jitter_comparison_0.png",
