...
def load_iperf3_results(test_path):
    ...
    for i in range(1, 12):
        try:
            ...
            print(f"JSON decode error in file {test_path}/tcp/sta{i}_iperf3.json: File may be empty or invalid.")
...
def plot_graph(x_values, y_values_dict, ylabel, title, filename, individual_results=None):
    ...
    plt.ylim(0, max([max(values['y']) for values in y_values_dict.values()]) + 1)
    plt.grid(True)
...
plot_graph(distances, {scheme: {'y': data['tcp_retrans'], 'color': data['color']} for scheme, data in data_to_plot.items()},
           'Retransmissions', 'TCP Retransmissions vs Distance from AP', f"{base_dir}/tcp_retransmissions_comparison.pdf", 
...
plot_graph(distances, {scheme: {'y': data['udp_throughput'], 'color': data['color']} for scheme, data in data_to_plot.items()},
           'Throughput (Mbps)', 'UDP Throughput vs Distance from AP', f"{base_dir}/udp_throughput_comparison.pdf", 
...
plot_graph(distances, {scheme: {'y': data['udp_loss'], 'color': data['color']} for scheme, data in data_to_plot.items()},
           'Packet Loss (%)', 'UDP Packet Loss vs Distance from AP', f"{base_dir}/udp_packet_loss_comparison.pdf", 
...
plot_graph(distances, {scheme: {'y': data['udp_jitter'], 'color': data['color']} for scheme, data in data_to_plot.items()},
           'Jitter (ms)', 'UDP Jitter vs Distance from AP', f"{base_dir}/udp_jitter_comparison.pdf", 
